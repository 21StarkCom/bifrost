...
`--agnes` makes the first message just `/agnes STARK-n`. It shipped in the
hermod release after v0.19.0 (STARK-6974); on v0.19.0 or older the same launch
...
3. Print the ack's `surface`, `workspace`, `name` and `prompt`, and stop. The
   `prompt` must read `/agnes STARK-n` (`$agnes STARK-n` on Codex) — that line is
   the whole hand-off.
