...
overrides:
  codex:
    argument-hint: <spec-path> [--dry-run] [--no-advisory] [--no-fix] [--stay-draft] [--agent auto|claude|codex|gemini] [--max-turns N] [--model ID]
    description: 'Stage 2 — autonomous implementation from an accepted stark-author spec: one fresh isolated writer per task, runner-owned immutable checks, evidence per task, commit per green task, held-out e2e gate, one cross-vendor advisory review, ONE bounded fix round for medium+ findings, draft PR. No LLM review loops. Use for build, implement a spec.'
    disable-model-invocation: true
    name: stark-build
    body: |
      ...
      # stark-build — Stage 2: fresh writer per task, runner-owned checks

      You are the RUNNER: an interactive session that sets up the harness, then
      dispatches one fresh isolated writer per spec task and verifies each result
      deterministically. The current host coordinates the run; the writer may be a
      native isolated worker or an enabled headless agent CLI. The implementer never
      grades itself.

      Evidence base: [references/stage2-dossier.md](references/stage2-dossier.md)
      (`[RQn]`/`[§n]` tags cite it). Consumes a spec accepted by the
      `stark-author` skill.
      ...
      **Non-negotiables** [§1]:
      - **Completion is a runnable pass/fail check, never a model verdict.** Agents
        ...
        "done" claim is noise.
      - **Checks are read-only to the writer, enforced by capabilities, not prompts.**
        For a hook-capable Claude writer, use the existing path-deny and Stop hooks.
        For any other writer, keep harness state outside its writable root, restrict
        writes to the task worktree, snapshot protected paths before dispatch, and
        reject/reset the result if the protected-path diff is non-empty. If the host
        cannot enforce that boundary and verify it afterwards, stop rather than run
        an unprotected autonomous build. Anti-cheat prompting alone fails (80–95%
        still hacked under explicit instructions).
      ...
      - **One task per fresh session.** Spec + the runner-injected PROGRESS summary +
        git are the only cross-session memory. The worker never writes runner state.
        [RQ3]
      ...
      **Harness subprocess rule — stdin closed, always.** Every dispatched headless
      agent subprocess must have `</dev/null` before its
      output redirect. **A hung reviewer is indistinguishable from a thorough
      one.** Agent CLIs may read stdin *in addition to* the prompt argument, so an
      open pipe that never delivers EOF — which is what stdin is under a backgrounded
      or orchestrated shell — can block them. Pass prompts as arguments or staged
      files, then add `</dev/null` before `>` on every dispatch below; any new
      dispatch inherits this rule.

      The rule covers **every** process this skill starts, not just the LLM
      dispatches:
      - **Claude hooks, when selected** — `stop-gate.sh` runs the task's check with stdin closed,
        because a Stop hook's own stdin is the host payload pipe and a
      ...
      Parse the spec path and options directly from the user's current request after
      the explicitly invoked skill name. Resolve `--agent auto` to an available fresh
      writer, preferring the current host's isolated worker or CLI. Resolve model IDs
      from runtime configuration unless the user supplied `--model`; never hardcode a
      vendor model.
      ...
      1. Resolve `SPEC` to an absolute path; it must contain `accepted-base: <sha>`
         ...
         the `stark-author` skill). Derive `SLUG` from the spec filename
      ...
      2. Parse `## Tasks`: per task — id, title, declared file set, done-when
         ...
         success (return to the `stark-author` skill). Topo-sort by declared edges;
      ...
      State lives OUTSIDE the repo and outside the writer's writable root:
      `STATE=${STARK_STATE_ROOT:-$HOME/.stark/code-review}/history/build/<slug>/<run-id>/`
      (`run-id` = UTC timestamp). Set `umask 077`, then let the runner create
      `$STATE/{hooks,tasks,evidence}` and every file below them. Never place `STATE`
      under the task worktree. The runner is the sole writer of this tree: workers
      emit stdout markers; they never create, append, truncate, chmod, rename, or
      delete runner state.
      ...
      1. **Worktree at the pin:** `git worktree add <wt> <accepted-base>`, then
         ...
         ```bash
         set -euo pipefail
         if [ -f "skill/stark-build/SKILL.md" ] && [ -f "tools/copilot_land.ts" ]; then
           ASSET_ROOT="$(pwd)"
         else
         ...
         git -C "$WT" rev-parse --abbrev-ref HEAD | grep -qx "build/$SLUG" \
           || { echo "HARD STOP: worktree is detached or on the wrong branch." >&2; exit 1; }
         git -C "$WT" merge-base --is-ancestor "$ACCEPTED_BASE" HEAD \
           || { echo "HARD STOP: HEAD does not contain accepted-base." >&2; exit 1; }
         ```
         ...
         **Check the exit code FIRST, then assert.** The self-contained command above
         does both. A refusal is the guard
      ...
      3. **Harness files:** copy this skill's
         ...
         `$STATE/hooks/`, `chmod +x`, and create `$STATE/PROGRESS.md` and
         `$STATE/protected.list` mode `0600`. Write one absolute path per line to the
         protected list, with **no trailing slash on directory entries**. Its first
         entry is `$STATE` itself, which protects the list, `PROGRESS.md`, evidence,
         hooks, task checks/settings/prompts/logs/counters, advisory artifacts, and
         every later runner-created descendant. Then list the spec + `.human.md`
         sidecar, every EXISTING test named by any done-when, and the repo's CI config
         dir. [RQ1/RQ5 — read-only tests, measured]

         For a Claude writer, materialize `settings.json` with the hooks below. For a
         non-Claude writer, do not pass Claude settings: keep `STATE` outside its
         writable sandbox. **For every writer, including Claude**, compute a
         deterministic manifest of every descendant under `$STATE` plus every
         protected repo/spec path before dispatch: relative path, type, mode, symlink
         target, and SHA-256 content for regular files. Hold that manifest in the
         runner process, never in a worker-visible file or prompt. After dispatch,
         require an exact match except for the predeclared runner output channels
         (`$T/session.log` and the hook-owned numeric `$T/blocks`). Validate those two
         separately, then take a new full manifest after each runner-owned append.
         Any other added, removed, content-changed, mode-changed, or symlinked state
         path is harness tampering and aborts the run. Hash evidence and PROGRESS too;
         they are not trusted merely because the runner intends to write them.
      ...
      4. **Initialize `$STATE/PROGRESS.md`**. Only the runner appends canonical
         per-task status, evidence hashes/pointers, deviations, and fix dispositions.
         Recompute the runner-held state manifest after every append. [RQ3]
      ...
      **Prepare** (`T=$STATE/tasks/<id>`): write `$T/check.sh` = the task's
      ...
      fails-on-success gate: stop, never silently rewrite. When the writer is Claude,
      also write `$T/settings.json`:

      ```json
      {"hooks": {
        "PreToolUse": [{"matcher": "Edit|Write|NotebookEdit|Bash",
          "hooks": [{"type": "command", "command": "$STATE/hooks/protect-paths.sh $STATE/protected.list <id>"}]}],
        "Stop": [{"hooks": [{"type": "command",
          "command": "$STATE/hooks/stop-gate.sh $T/check.sh $T/blocks $T/session.log <id>"}]}]
      }}
      ```

      (absolute, shell-quoted paths; `printf '0' > "$T/blocks"` and pre-create mode
      `0600` `$T/session.log`). Other writers do not receive this Claude-specific
      settings file. For every writer, create `$T/prompt.md` from this template — fill
      every `<>`:

      > Implement exactly ONE task of an accepted spec. Repo: `<wt>`. Spec (read
      > it, treat as immutable): `<SPEC>`. Task: `<id> — <title>`. Declared files:
      > `<file set>`. Done-when: `<command>`. Progress summary: `<runner-provided
      > prior-task status and deviations>`.
      > 1. Orient: read the spec's task, the progress summary embedded below, and
      ...
      > 3. Implement until the done-when passes. Gated files are runner-protected —
      >    add NEW tests freely, never edit existing ones. The runner rejects any
      >    protected-path change even when this host has no live hook.
      > 4. Evidence: run checks normally and include their output in your response.
      >    The runner captures the whole session log under its private state tree.
      >    Never write outside `<wt>`.
      > 5. Touch only the declared files. If you must touch outside the set,
      >    print exactly this one-line marker on stdout and stop:
      >    `STARK_DEVIATION task=<id> class=<blocked|spec-defect|scope-move> message=<single-line reason>`
      >    Use no fence, prefix, suffix, control character, or extra marker. The
      >    reason must be 1–500 characters. The runner validates the fields and
      >    appends a canonical entry to PROGRESS.md.
      > 6. Finish with exactly one commit: `build(<slug>): <id> <title>`.
      > 7. Abort is success: if blocked, print the deviation marker and stop. The gate
      >    accepts a logged deviation — never fake a green.

      `STARK_DEVIATION` is the only task-session control marker. A worker must never
      write `protected.list`, `PROGRESS.md`, `evidence/`, `hooks/`, or `tasks/`
      directly. The runner accepts at most one anchored marker for the current task;
      the id must equal `<id>`, class must be one of the three literals above, and the
      reason must be a printable single line of 1–500 characters. Multiple, malformed,
      wrong-task, or control-character-bearing markers are a blocked result. Treat the
      line as data: never `eval` it or interpolate it into a command.

      **Dispatch** (fresh session; budget per task, not per run [§3.3]):

      Before dispatch, embed the current runner-owned PROGRESS summary in the prompt.
      Use the host's native fresh writable worker only if it supports no inherited
      conversation, a writable root limited to `WT`, and no write access to `STATE`.
      Otherwise use an enabled headless CLI. The following self-contained fallback
      supports the installed Claude, Codex, and Gemini CLIs. Replace placeholders from
      the parsed run state before executing. Claude receives the hook settings; Codex
      and Gemini rely on workspace isolation plus the mandatory post-dispatch tamper
      gate.

      ```bash
      set -euo pipefail
      AGENT="<resolved-agent>"
      MODEL="<configured-or-user-model>"
      MAX_TURNS="<max-turns-or-30>"
      WT="<absolute-worktree-path>"
      T="<absolute-task-state-path>"
      case "$AGENT" in
        claude)
          claude -p --model "$MODEL" --settings "$T/settings.json" \
            --max-turns "$MAX_TURNS" --dangerously-skip-permissions \
            "$(cat "$T/prompt.md")" </dev/null > "$T/session.log" 2>&1
          ;;
        codex)
          codex exec -C "$WT" --full-auto --ephemeral -m "$MODEL" \
            "$(cat "$T/prompt.md")" </dev/null > "$T/session.log" 2>&1
          ;;
        gemini)
          (cd "$WT" && gemini -m "$MODEL" -p "$(cat "$T/prompt.md")" \
            --sandbox --approval-mode auto_edit --skip-trust \
            </dev/null > "$T/session.log" 2>&1)
          ;;
        *) echo "No supported isolated writer is available" >&2; exit 1 ;;
      esac
      ```

      **Verify (you, deterministically — never ask the session how it went):**
      Before interpreting any worker output, compare the runner-held pre/post state
      manifests. Only `$T/session.log` and a numeric `$T/blocks` may differ during the
      dispatch; every other state/protected-path delta is a hard tamper abort. The
      runner then copies and hashes `$T/session.log` as worker evidence. For every
      runner-owned gate, the runner captures stdout/stderr under
      `$STATE/evidence/<id>.log` while preserving the gate's exact exit status, then
      refreshes the full state manifest.
      1. Gate: run `bash "$T/check.sh" </dev/null` yourself → green / red.
         ...
         **Wrapper flake fallback.** If `bash "$T/check.sh"` does not return, the
         ...
         flaky discipline applies), and have the runner record in PROGRESS.md that
         the wrapper did not return, that direct invocation was used, and both
         outcomes.
      ...
      2. Tamper: `git -C <wt> diff --name-only <pre-sha>..HEAD` (plus dirty files)
         ∩ the protected repo/spec entries must be EMPTY. The full runner-state
         comparison described above applies to **every** writer, including Claude;
         never waive it because hooks were installed. A hit aborts the whole run
         (`class=blocked`, runner-authored harness-tamper note, straight to Phase 5).
      3. Scope: same diff vs declared file set → out-of-set paths get a deviation
         line. Parse only a complete line matching
         `^STARK_DEVIATION task=<id> class=(blocked|spec-defect|scope-move) message=.{1,500}$`
         from the worker log. Require the literal current task id, printable text,
         and exactly one marker; reject malformed or conflicting markers. The runner
         writes the canonical PROGRESS entry itself and never executes marker text.
      4. Commit: green + uncommitted → commit `build(<slug>): <id> (runner-committed)`;
         ...
         is durable. [RQ8]
      5. The runner appends the task entry to PROGRESS.md: status (`green | deviation
         | crashed`), evidence path + SHA-256, blocks count; then refreshes the state
         manifest.
      6. Crash/timeout without green or deviation → re-dispatch the SAME task ONCE
         ...
         exit → the runner writes a `class=blocked` deviation and moves on.
      ...
      **Phase 2 §Verify 1's execution rules apply here in full**: run it with
      ...
      - **Red with all tasks green** → the divergence signal (build gamed or spec
        mis-decomposed [RQ9]): the runner appends it to PROGRESS.md, refreshes the
        state manifest, continues to Phase 4, keeps the PR DRAFT, and says it plainly
        in the report. There is NO fix loop — the human decides.
      ...
      Choose one enabled reviewer from a different vendor than the task writer. Honor
      runtime configuration; do not hardcode Codex when Codex was the writer or host.
      Use a native zero-context read-only worker when the host provides one, otherwise
      use an installed headless reviewer CLI with writes disabled. If no different
      vendor can run read-only, record `advisory=unavailable`, leave the PR draft, and
      stop before any fix round; never relabel a same-vendor self-review as
      cross-vendor.

      Stage the final diff and advisory prompt, then dispatch with a self-contained
      call. This CLI fallback covers Claude and Codex; use a host-native isolated
      reviewer for Gemini unless the runtime can provision a read-only Gemini home:

      ```bash
      set -euo pipefail
      REVIEWER="<different-vendor-reviewer>"
      MODEL="<configured-reviewer-model>"
      WT="<absolute-worktree-path>"
      ACCEPTED_BASE="<accepted-base-sha>"
      STATE="<absolute-run-state-path>"
      git -C "$WT" diff "$ACCEPTED_BASE"..HEAD > "$STATE/final.diff"
      case "$REVIEWER" in
        codex)
          codex exec -C "$WT" -s read-only --ephemeral -m "$MODEL" \
            "$(cat "$STATE/advisory-prompt.md")" </dev/null > "$STATE/advisory.out" 2>&1
          ;;
        claude)
          claude -p --model "$MODEL" --allowedTools "Read,Glob,Grep" \
            "$(cat "$STATE/advisory-prompt.md")" </dev/null > "$STATE/advisory.out" 2>&1
          ;;
        *) echo "Use a host-native zero-context read-only reviewer for $REVIEWER" >&2; exit 2 ;;
      esac
      ```

      The runner creates `final.diff`, `advisory-prompt.md`, and `advisory.out`, hashes
      them, and refreshes the state manifest. The reviewer receives prompt bytes, not
      write access to `$STATE`; the output redirect is the only permitted state delta
      ...
      3. **Dispatch ONE fresh writer** — use the same resolved writer and the same
         portable dispatch procedure as Phase 2. Create an absolute
         `$STATE/tasks/fix` directory, write its check as the spec's `## Verification`
         command(s) verbatim, and use that check as the terminal gate. A Claude writer
         receives the path-deny/Stop settings; any other writer receives a worktree-
         only writable sandbox and the runner's before/after protected-path gate.
         Close stdin and write its output to `session.log`, exactly as in Phase 2.
         ...
         file no finding names. A finding you judge WRONG is not fixed — print the
         exact rejection marker described below and move on; do not write runner
         state. Rejecting on evidence is a success, faking a fix is not. Finish with
         exactly one commit: `fix(<slug>): advisory round`."*

         Include this exact one-line grammar in the rendered prompt:
         `STARK_FINDING_REJECTED task=fix id=<n> reason=<single-line reason>`.
         The runner accepts only complete lines with `task=fix`, an id in the selected
         findings set, and a printable 1–500 character reason; duplicate, unknown,
         malformed, or conflicting ids are blocked results. Only the runner appends
         validated rejections to PROGRESS.md. Never evaluate marker text.
      4. **Verify (you, deterministically — the reviewer is not consulted again):**
         ...
         Phase 2 §Verify, including the every-writer state manifest comparison.
      5. **Record.** The runner appends to PROGRESS.md: findings selected, fixed,
         validated rejected markers (with reasons), and gate outcome, then refreshes
         the state manifest. Post ONE follow-up PR comment listing what
         was fixed, what was rejected and why, and what remains open — the
      ...
      **fixed / rejected-with-reason / still-open** · evidence dir · e2e verdict ·
      ...
      For implementation from an accepted spec, this replaces the `stark-copilot`
...
---
Usage: stark-build <spec-path> [--dry-run] [--no-advisory] [--no-fix] [--stay-draft] [--max-turns N] [--model ID]
