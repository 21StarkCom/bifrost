...
6. **Self-check.** (a) Would the laziest change passing every Acceptance line meet the Goal? If not, add the line it fails. (b) Apply the fix and each Acceptance line literally to the incident. (c) Cross-read Goal, Scope IN/OUT and Acceptance: no INVARIANT the Goal breaks, no check the planned change cannot pass. (d) Every Scope IN line traces to the Ask; the rest is TRIM. (e) A trivial-change ticket stays under 250 words; no `TODO:` left but Cold read.
