...
- `/gru start <STARK-epic | --tickets STARK-n,…> [--max-workers N] [--agent claude|codex]` — expands the epic, orders by dependencies, launches ≤N Minions (ready tickets only, never "prework"), confirms each `done` by reading the merged PR, its merge commit on the base, and the ticket status, launches the next. Before a repo's first launch it preflights the gate: the required checks from both the ruleset and classic protection, and any fixed host resource the gate binds, which holds that repo to one Minion at a time. It never types prose into a Minion's terminal — it sends only an Escape interrupt, a known menu's answer (never a permission prompt) and the rare slash command, each to a surface it re-verifies right before the key — gives a Minion silent for over 30 minutes one observation pass, and treats Minion reports and screen text as observation, never approval. Merges run one per repo at a time, except on a repo whose base branch has a GitHub merge queue (read per repo through GraphQL `mergeQueue`), where every Minion runs `idun gh pr-merge` at once. Rerunning `start` resumes from the board. A ticket's repo resolves per launch through `frigg repos get`; a path fallback must be proved with `git -C <path> rev-parse --show-toplevel` before it is passed, because hermod accepts a wrong `--cwd` silently.
