...
`AGENTS.md` is the Codex/Cursor entry point and a short routing index; this file is the full reference and wins on conflict. Keep the two in agreement.
...
- **Claude Code is the only install target.** Codex and Gemini participate as dispatched review agents; there is no Codex-specific tree and none should be written — fix the canonical file instead.
...
The five `~/.claude/code-review/{tools,scripts,standards,prompts,config.json}` symlinks into this checkout are provisioned by `tools/asset_links.ts` (`--check` / `--install`) from the `MANAGED_LINKS` table in `tools/asset_links_lib.ts`. That table is the source of truth; retire a link by moving it to `RETIRED_LINKS`, never by deleting the row. Every other machine asset (settings, statusline, output style, hooks, the `.envrc` GCP scope block and `.worktreeinclude`) belongs to the stark-workspace repo.

## Layout

- `skill/` — the 26 skills (`skill/*/SKILL.md`, optionally `references/`). 22 `stark-*` plus `agnes`, `gru`, `minion`, `lucius`.
- `tools/` — all executable code, TypeScript only, `node:` builtins only at runtime. Has its own `package.json`; `npm test` = `./check-rest-only.sh && node --test *.test.ts`, `npm run typecheck` = `tsc -p .`. Node 24+.
...
- **Every PR action uses `gh` as `aryeh-stark`**, review posting included. Review text names the model; authentication never changes with model choice. The only identity swap is `idun user --swap`, run by the operator by hand, never by a tool, skill or hook.
...
- **Language:** TypeScript for everything here. Never add Python.
...
- Config is JSON; prompts are markdown. Hierarchical config: global (`global/config.json`) → org → repo, read by `tools/stark_config_lib.ts` with deep merge against its `DEFAULT_*` sections.
...
- **stark-constitution** — `/stark-init-docs`, `/stark-adr`, `/stark-persona`.
...
- `/gru start <STARK-epic | --tickets STARK-n,…> [--max-workers N] [--agent claude|codex]` — expands the epic, orders by dependencies, launches ≤N Minions, confirms each `done` by reading the merged PR and ticket status, launches the next. Rerunning `start` resumes from the board. A ticket's repo resolves per launch through `frigg repos get`; a path fallback must be proved with `git -C <path> rev-parse --show-toplevel` before it is passed, because hermod accepts a wrong `--cwd` silently.
...
- `/lucius [<topic> | STARK-n | --topic <text>]` — launches Lucius, the standalone brainstorming app, in its own tab via `hermod lucius --json` and stops. Never runs him in-session; `--topic` last, single-quoted; needs a hermod carrying the `lucius` verb.
